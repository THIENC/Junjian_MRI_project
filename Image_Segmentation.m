...
for k = 1 : length(theFiles)
  ...
  spectralImage = dicomread(fullFileName);
...
% end
numberOfClasses = 9; % Assume 6 for demo.
%----------------------------------------------------------------------------------------
...
class7 = reshape(indexes == 7, rows, columns);
class8 = reshape(indexes == 8, rows, columns);
class9 = reshape(indexes == 9, rows, columns);


% Let's put these into a 3-D array for later to make it easy to display them all with a loop.
allClasses = cat(3, class1, class2, class3, class4, class5, class6,class7,class8,class9);
